...
namespace Icebox
{
	public class SimBox
	{
		...
		public void Databind(string id, SimVector3 worldPosition, Simulation simulation, int gridSizeX, int gridSizeY)
		{
			this.id = id;
		...
		public SimMap AddMap(SimMapType mapType)
		{
			...
			map.Databind(mapType, this, gridSizeX, gridSizeY);
		...
		public SimUnit AddUnit(SimUnitType unitType, SimPoint position)
		{
			...
			unit.Databind(unitType, nextUnitId++, position);
		...
		public SimAgent AddAgent(SimAgentType agentType, SimPoint position, SimUnit owner, SimResourceBinCollection resources, string searchTarget)
		{
			...
			agent.Databind(agentType, nextAgentId++, position, owner, resources, searchTarget);
